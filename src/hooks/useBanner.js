...
export const useBanner = () => {
  ...
  const fetchBanners = useCallback(async (storeId) => {
    ...
    try {
      ...
      if (responseData && responseData.success === true) {
        ...
        } else if (responseData.data === "No banners found" || responseData.data === "Banners fetched successfully from cache") {
  ...
  const updateBannerStatus = async (bannerId, isActive, storeId) => {
    try {
      const response = await axiosInstance.put(`/api/v1/banner/update-status/${bannerId}`, { isActive });
