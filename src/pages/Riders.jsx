...
import { PlusCircle, Search, Users, ClipboardList, CheckCircle2, XCircle, MoreHorizontal, User, Mail, Phone, Truck, CreditCard } from 'lucide-react'
...
import { useAllRiderStore } from '../store/allRiderStore.js';
...
export default function RidersPage() {
    ...
    const getRiders = async () => {
      try {
        ...
        if (response.status === 200) {
          ...
          const ridersData = Array.isArray(response.data) ? response.data : 
                            (response.data && response.data.riders ? response.data.riders : []);
  ...
  const filteredRiders = Array.isArray(allRiders) ? allRiders.filter(rider => 
  ...
  const filteredRejectedRiders = Array.isArray(rejectedRiders) ? rejectedRiders.filter(rider => 
    (rider.fullName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
     rider.email?.toLowerCase().includes(searchTerm.toLowerCase()))
  ) : [];

  // Calculate counts
  const pendingCount = Array.isArray(allRiders) ? allRiders.filter(rider => rider?.isApproved === false).length : 0;
                ...
                {filteredRiders.length > 0 ? (
                  filteredRiders.map((rider) => (
                    ...
                    >
                      ...
                      <TableCell>
                        <Badge
                          variant={rider.isAvailable === true ? "default" : 
                                  rider.isAvailable === false ? "secondary" : "outline"}
                        >
                      ...
                      <TableCell>
                        <Badge
                          variant={rider.isApproved === true ? "default" : "outline"}
                        >
                      ...
                      <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem onClick={() => navigator.clipboard.writeText(rider._id || rider.id)}>
                              Copy rider ID
                            </DropdownMenuItem>
                            <DropdownMenuSeparator />
                            <DropdownMenuItem onClick={() => handleRiderClick(rider)}>View details</DropdownMenuItem>
                  ...
                  <TableRow>
                    <TableCell colSpan={8} className="text-center py-4">No riders found</TableCell>
                  </TableRow>
                )}
              </TableBody>
            </Table>
            ...
            <Table>
              ...
              <TableBody>
                {filteredRejectedRiders.length > 0 ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-4">No rejected riders found</TableCell>
                  </TableRow>
                )}
              </TableBody>
            </Table>
          ...
          {selectedRider && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  View and verify rider information
                </DialogDescription>
              </DialogHeader>
