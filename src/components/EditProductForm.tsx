...
export function EditProductForm({ product, onClose, onSuccess }: {
  ...
  useEffect(() => {
    if (product) {
      ...
      if (product.productImages) {
        setExistingImages(product.productImages.map((img: any) => img.imageUrl));
      }
    }
  }, [product, form]);
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newImages = files.filter(
              (file: any) => ['image/jpeg', 'image/png', 'image/webp'].includes(file.type) && file.size <= 5000000
    );
    ...
    setProductImages((prev: any[]) => [...prev, ...limitedImages]);
  };
